...
class Game:
    def __init__(self):
        ...
        self.message = "Enter moves here Example: a2 a3"
        self.gameboard = {}
        self.placePieces()
        print("Welcome left is white and right is black")
        print("White's turn first")
        self.main()
    def placePieces(self):

        for i in range(0,8):
            self.gameboard[(i,1)] = Pawn(BLACK,PieceDict[BLACK][Pawn],1)
            self.gameboard[(i,6)] = Pawn(WHITE,PieceDict[WHITE][Pawn],-1)

        placers = [Rook,Knight,Bishop,Queen,King,Bishop,Knight,Rook]

        for i in range(0,8):
            self.gameboard[(i,0)] = placers[i](BLACK,PieceDict[BLACK][placers[i]])
            self.gameboard[((7-i),7)] = placers[i](WHITE,PieceDict[WHITE][placers[i]])
        placers.reverse()
...
PieceDict = {WHITE : {Pawn : "♙", Rook : "♖", Knight : "♘", Bishop : "♗", King : "♔", Queen : "♕" }, BLACK : {Pawn : "♟", Rook : "♜", Knight : "♞", Bishop : "♝", King : "♚", Queen : "♛" }}
Game() 
